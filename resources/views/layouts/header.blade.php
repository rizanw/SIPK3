<div class="c-wrapper">
    <header class="c-header c-header-light c-header-fixed c-header-with-subheader">
        ...
        <a class="c-header-brand d-sm-none" href="#"><img class="c-header-brand"
                                                          src="{{ env('APP_URL', '') }}/assets/brand/coreui-base.svg"
        ...
        <ul class="c-header-nav ml-auto mr-4">
            <li class="c-header-nav-item dropdown">
                <a class="c-header-nav-link" style="padding-right: 4.5rem"
                   data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
                    <div class="c-avatar">
                        <img class="c-avatar-img"
                             src="{{ env('APP_URL', '') }}/assets/img/avatars/7.jpg"
                             alt="{{{ isset(Auth::user()->name) ? Auth::user()->name : Auth::user()->email }}}">
                        <div>{{{ isset(Auth::user()->name) ? Auth::user()->name : Auth::user()->email }}}</div>
                    </div>
                </a>
